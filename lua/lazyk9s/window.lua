...
local function open_floating_window()
	local floating_window_scaling_factor = vim.g.lazyk9s_floating_window_scaling_factor
	...
	if status and vim.g.lazyk9s_floating_window_use_plenary and vim.g.lazyk9s_floating_window_use_plenary ~= 0 then
		local ret = plenary.percentage_range_window(
			floating_window_scaling_factor,
			floating_window_scaling_factor,
			{ winblend = vim.g.lazyk9s_floating_window_winblend }
		)
	...
	local window_chars = vim.g.lazyk9s_floating_window_border_chars
	...
	vim.api.nvim_set_hl(0, "LazyK9sBorder", { link = "Normal", default = true })
	vim.cmd("set winhl=NormalFloat:LazyK9sBorder")

	-- create a unlisted scratch buffer
	if LAZYK9S_BUFFER == nil or vim.fn.bufwinnr(LAZYK9S_BUFFER) == -1 then
		LAZYK9S_BUFFER = api.nvim_create_buf(false, true)
	else
		LAZYK9S_LOADED = true
	end
	-- create file window, enter the window, and use the options defined in opts
	local win = api.nvim_open_win(LAZYK9S_BUFFER, true, opts)

	vim.bo[LAZYK9S_BUFFER].filetype = "lazyk9s"

	vim.cmd("setlocal bufhidden=hide")
	vim.cmd("setlocal nocursorcolumn")
	vim.api.nvim_set_hl(0, "LazyK9sFloat", { link = "Normal", default = true })
	vim.cmd("setlocal winhl=NormalFloat:LazyK9sFloat")
	vim.cmd("set winblend=" .. vim.g.lazyk9s_floating_window_winblend)
